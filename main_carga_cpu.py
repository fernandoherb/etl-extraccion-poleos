...
def main():
    ...
    try:
        ...
        carga_cpu  = extract_process()
        fin = time.time()
        logger.info('fin %s',str(fin))
        logger.info('Tiempo de ejecucion: %s',str(fin-inicio))
        logger.info('Pruebas de conexión y extracción carga_cpu: %s',str(len(carga_cpu)))
        ## Procesando tabla de leads_lead
        logger.info(carga_cpu.head(1))
        ...
        database.execute_delete_postg("DELETE FROM historico.detalle_carga_cpu_7d WHERE fecha_creacion < CURRENT_TIMESTAMP - INTERVAL '24 hours';")  

...
def main():
    ...
    try:
        ...
        error_interface  = extract_process()
        fin = time.time()
        logger.info('fin %s',str(fin))
        logger.info('Tiempo de ejecucion: %s',str(fin-inicio))
        logger.info('Pruebas de conexión y extracción error_interface: %s',str(len(error_interface)))
        ## Procesando tabla de leads_lead
        logger.info(error_interface.head(1))
...
#        str_date = str(yesterday)
        ...
        error_interface = database.select_data_frame(database.query_select_detalle_error_interface())
...
def transform_process(data_frame, fields):
    ...
    try:
        ...
        ct_error_interfaces = database.execute_select_query_pandas(database.query_select_ct_errores_interface())
        # Encuentra los nodos faltantes usando `merge`
        nodos_faltantes = ct_error_interfaces[~ct_error_interfaces['interface_id'].isin(data_frame['interface_id'])]
        
        logger.info('Interfaces faltantes:::::: '+str(len(nodos_faltantes)))
                
        # Concatenamos `df_poleos` con `ct_error_interfaces_faltantes`
